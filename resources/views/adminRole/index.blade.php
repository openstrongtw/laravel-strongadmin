...
<script type="text/html" id="ST-TOOL-BAR">
    <div class="layui-btn-container st-tool-bar">
        <a class="layui-btn layui-btn-xs" onclick="Util.createFormWindow('/strongadmin/adminRole/create', this.innerText);">新增</a>
        <a class="layui-btn layui-btn-xs" lay-event="batchDelete" data-href="/admin/adminRole/destroy">刪除選中</a>
    </div>
</script>
<script type="text/html" id="ST-OP-BUTTON">
    @verbatim
    <a class="layui-btn layui-btn-xs" onclick="Util.createFormWindow('/strongadmin/adminRole/assignPermissions?id={{d.id}}', this.innerText);">分配許可權</a>
    <a class="layui-btn layui-btn-xs" onclick="Util.createFormWindow('/strongadmin/adminRole/update?id={{d.id}}', this.innerText);">更新</a>
    <a class="layui-btn layui-btn-danger layui-btn-xs" onclick="Util.destroy('/strongadmin/adminRole/destroy?id={{d.id}}');">刪除</a>
    @endverbatim
</script>
@endsection
@push('scripts_bottom')        
...
!function () {
    ...
    var cols = [
                ...
                , {field: 'id', title: 'id', width: 80, fixed: 'left', unresize: true, totalRowText: '合計', sort: true}
