...
<script type="text/html" id="ST-TOOL-BAR">
    <div class="layui-btn-container st-tool-bar">
        <a class="layui-btn layui-btn-xs" onclick="Util.createFormWindow('/strongadmin/adminUser/create', this.innerText);">新增</a>
        <a class="layui-btn layui-btn-xs" lay-event="batchDelete" data-href="/admin/adminUser/destroy">刪除選中</a>
    </div>
</script>
<script type="text/html" id="ST-OP-BUTTON">
    @verbatim
    <a class="layui-btn layui-btn-xs" onclick="Util.createFormWindow('/strongadmin/adminUser/update?id={{d.id}}', this.innerText);">分配角色</a>
    <a class="layui-btn layui-btn-xs" onclick="Util.createFormWindow('/strongadmin/adminUser/update?id={{d.id}}', this.innerText);">更新</a>
    <a class="layui-btn layui-btn-danger layui-btn-xs" onclick="Util.destroy('/strongadmin/adminUser/destroy?id={{d.id}}');">刪除</a>
    @endverbatim
</script>
@endsection
@push('scripts_bottom')        
...
!function () {
    ...
    var cols = [
                ...
                , {field: 'status', title: '{{$model->getAttributeLabel("status")}}', width: 80, templet: function (res) {
                    return  res.status==1  ? '<button class="layui-btn layui-btn-xs st-btn-bg-succ">開啟</button>' : '<button class="layui-btn layui-btn-xs layui-btn-radius layui-btn-danger">禁用</button>';
                    }}
